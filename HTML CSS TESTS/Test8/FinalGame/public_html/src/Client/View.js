...
var testFunction = function() {
    window["ClassA"] = function() {
        eval("this.mNumber = 30; this.mPi = 3.14;");
    };
    ...
    window["ClassA"].prototype["test"] = new Function("console.log(2);");
                // 2
    
    var code = window["ClassA"];
    //console.log(code);
    
    // Concenate every method's code
    for(var functionName in window["ClassA"].prototype) {
        code += "\n\n// Function Name: " + functionName + "\n" + window["ClassA"].prototype[functionName];
    }
    console.log(code);
    ...
    // Define ClassB
    window["ClassB"] = function() {
        eval("ClassA.call(this); this.mNumber = 60;");
        // I'm calling this.mNumber = 60 after calling classA's constructor, so it doesn't get reset to 30
    };
    
    // Call this to create inheritance
    gEngine.Core.inheritPrototype(window["ClassB"], window["ClassA"]);
    
    // Instantiate ClassB
    var b = new ClassB();
         
    window["ClassB"].prototype["test2"] = new Function("console.log(1);");
    window["ClassB"].prototype["test3"] = new Function("x", "y", "console.log(x+y);");
    
    var code2 = window["ClassB"];
    //console.log(code);
    
    // Concenate every method's code
    for(var functionName in window["ClassB"].prototype) {
        code2 += "\n\n// Function Name: " + functionName + "\n" + window["ClassB"].prototype[functionName];
    }
    console.log(code2);
    b.test3(5,3);
    ...
    /*
    console.log(code);
    window["ClassA"] = code; //conclusion: need to manually parse
    // as well as manually create method headers or delete anonymous function
    //headers.
    // cannot send back code all at once.
    console.log("ok done");    
    ...
    //console.log(code);
    //console.log(window["ClassA"].prototype["test"]);
    /*
    // Define ClassB
    window["ClassB"] = function() {
        eval("ClassA.call(this); this.mNumber = 60;");
    ...
    var b = new ClassB();
                 // 2
    // Override it now
    window["ClassB"].prototype["test"] = new Function("console.log(1);");
    ...
    // Concenate every method's code
    for(var functionName in window["ClassB"].prototype) {
        code += window["ClassB"].prototype[functionName];
    }
    console.log(code);*/
};
