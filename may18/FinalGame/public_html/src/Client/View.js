...
var renameCode = function(obj, newName) {
    // Parses the object code to find class names, and replaces them with the new name
    
    // Deal with multi-line comments first
    var arr1 = obj[1].split("/*");
    var i;
    for (i = 0; i < arr1.length; i++) {
        var arr2 = arr1[i].split("*/");
        // All of arr2[0] is commented code, even if there are 2+ "/*"s in a row
        // If arr2[1] is a thing, then that's the actual code beyond the comment
        // There won't be an arr2[2] ever
        
        // i === 0 is not commented ever
        var arr3 = null;
        if (i === 0) {
            arr3 = arr2[0].split("//");
        ...
        // Now deal with regular comments (from // to the \n\ character)
        if (arr3 !== null) {
            var arr1 = obj[1].split("/*");
