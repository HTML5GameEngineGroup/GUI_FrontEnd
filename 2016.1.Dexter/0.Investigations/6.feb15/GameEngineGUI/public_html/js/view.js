...
$( "#playbtn" ).click(function() {
    ...
    for(i = 0; i < GlobalInstanceList.length; i++){
        //GlobalInstanceList[i].runAll(functions);
        GlobalInstanceList[i].play();
    }
});
...
$( "#newobject" ).click(function() {
    ...
    var defaultObjectName = 'GameObj' + GlobalPGOList.length;
        ...
        functionNameOK.click(function() {
            ...
            if (selected === null || selected instanceof GameObject) {
                ...
                if (typeof selected[textValue] === "undefined") {
                    selected[textValue] = new Function();/*function() {
    
};  // Doing it like this causes the curly brace and tab to appear in the same format*/
                }
                // Make the editor and pull up the function (which is now guaranteed to exist)
                var editorDiv = $('<div id="scriptEditor">' + selected[textValue].toString() + '</div>');
                editorArea.append(editorDiv);
                ...
                button.click(function() {
                    if (selected === null) {
                        ...
                        // Attach the function to each child of the parent
                        for (i = 0; i < GlobalInstanceList.length; i++) {
                            if (GlobalInstanceList[i].getParent() === selected) {
                                GlobalInstanceList[i][textValue] = new Function(substring);
                            }
                        }
                        
                        // Attach it to the parent as well
                        selected[textValue] = new Function(substring);
...
$( 'div' ).on('click', 'a.list-group-item', function() {
    ...
    button.click(function() {
        if (selected instanceof ParentGameObject) {
            // The button is removed when selected changes to an instance, but just to be sure for later, this will be checked (b/c new GO requires a PGO).
    ...
    selected = findPGOByName($(selectedListItem).text().trim());
...
var createDefaultGameObj = function (defaultName){
    var pgo = new ParentGameObject(defaultName);
    pgo.setColor([1,0,0,1]);
    pgo.getXform().setPosition(20, 60);
    pgo.getXform().setSize(5,5);
    pgo.getXform().setRotationInDegree(20);
    
    selected = pgo;
    
    GlobalPGOList[GlobalPGOList.length] = pgo;
...
var findPGOByName = function(name){
    var i;
    for(i = 0; i < GlobalPGOList.length; i++){
        if(GlobalPGOList[i].getName() === name){
            return GlobalPGOList[i];
        }
    }
    return null;
};
...
var changeInstanceTo = function(index) {
    selected = GlobalInstanceList[index];
    updateEditView(selected.getAllContents());
