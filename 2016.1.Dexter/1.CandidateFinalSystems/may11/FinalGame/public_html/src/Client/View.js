...
$('#menuFileNew').click(function(event) {
    ...
    gNextInstanceID = 0;        // ""
    gNextSceneID = 1;           // Since one scene is provided at the start
...
var fileOpen = function() {
    ...
    var input = document.getElementById("menuFileOpenInput").files[0];
    
    if (input) {
        // Only accept .zip
        if (input.name.endsWith(".zip")) { // Make sure it is in a .js format
            ...
            reader.onloadend = function() {
                //TODO WARNING MSG AND ABORT IF USER SAYS NO
...
$(document).on('contextmenu', 'li', function(event) {
    ...
    if (id.startsWith("objectListItem")) {
        ...
        $('#scenesMenu').finish().toggle().css({top: event.pageY + 'px', left: event.pageX + 'px'});
        gRightClickedItem = $(this).attr('id');
    } else if (id.startsWith("cameraListItem")) { 
        $('#camerasMenu').finish().toggle().css({top: event.pageY + 'px', left: event.pageX + 'px'});
        gRightClickedItem = $(this).attr('id');
    }
});
...
$('div').on('focusout', '.text-field', function() {
    ...
    switch($(this).attr('id')) {
        ...
        case "panelRightCamerasX":
            var cam = gGameCore.getSelectedCamera();
            cam.setWCCenter($(this).val(), cam.getWCCenter[1]);
            break;
        case "panelRightCamerasY":
            var cam = gGameCore.getSelectedCamera();
            cam.setWCCenter(cam.getWCCenter[0], $(this).val());
            break;
        ...
        case "panelRightCamerasVPX":
            ...
            cam.setViewport([$(this).val(), vp[1], vp[2], vp[3]]);
            break;
        case "panelRightCamerasVPY":
            var cam = gGameCore.getSelectedCamera();
            var vp = cam.getViewport();
            cam.setViewport([vp[0], $(this).val(), vp[2], vp[3]]);
            break;
        case "panelRightCamerasVPW":
            var cam = gGameCore.getSelectedCamera();
            var vp = cam.getViewport();
            cam.setViewport([vp[0], vp[1], $(this).val(), vp[3]]);
            break;
        case "panelRightCamerasVPH":
            var cam = gGameCore.getSelectedCamera();
            var vp = cam.getViewport();
            cam.setViewport([vp[0], vp[1], vp[2], $(this).val()]);
            break;
...
var createCodeEditor = function(selectedForCode) {
    ...
    $('#codeOK').click(function() {
        ...
        try {
            ...
            for (i = 0; i < scenes.length; i++) {
                ...
                for (j = 0; j < instances.length; j++) {
                    if (instances[j].mName === selectedForCode[0].mName) { // Cannot use instanceof to check the actual class name
                        if (instances[j] instanceof GameObject) { // Instanceof to check if it is a GO or class
                            ...
                            //var color = instances[j].getColor();
                            var id = instances[j].mID;
                            
                            // Re-create it
                            eval("instances[j] = new " + selectedForCode[0].mName + "(new Renderable());");
                            ...
                            instances[j].mName = selectedForCode[0].mName;
                            instances[j].mID = id;
                        } else {
                           // Get old values while you still can
                            var id = instances[j].mID;
        ...
        //console.log(window["GameObj0r"].prototype["update"]);
...
var createPanelBottomCameras = function() {
    ...
    if (gCurrentScene.mID === "sceneListItemBlank") {
        $('#panelBottom').append('<ul class="panel-body" id="panelBottomBody"></ul>');
        return;
    }
...
var cleanUpGameCore = function() {
    ...
    var item = gGameCore.getObjectList();
    item.splice(0, item.length);
    item = gGameCore.getSceneList();
    item.splice(0, item.length);
    item = gGameCore.getTextureList();
    item.splice(0, item.length);
    item = gGameCore.getSelected();
    item = null;
    gEngine.GameLoop.stop();
    gGameCore.runBlankScene();
    item = gGameCore.getSelectedForCode();
    item = null;
};
