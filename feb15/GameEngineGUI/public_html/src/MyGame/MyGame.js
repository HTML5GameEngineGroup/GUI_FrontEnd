...
/*global view: true, model: true, gEngine: false, Scene: false, BlueLevel: false, Camera: false, Renderable: false, vec2: false */
...
//    
     ...
     for (i = 0; i < GlobalInstanceList.length; i++) {
        GlobalInstanceList[i].draw(this.mCamera);
     } 
...
MyGame.prototype.update = function () {
    ...
    if (!GlobalPlay && gEngine.Input.isButtonClicked(gEngine.Input.mouseButton.Left)) {
        if (this.mCamera.isMouseInViewport()) {
            ...
            for (i = GlobalInstanceList.length - 1; i >= 0; i--) {
                var bboxObj = GlobalInstanceList[i].getBBox();
                if (bboxObj.boundCollideStatus(bboxMouse) > 0) {
                    // Got a collision between mouse and object
                    changeInstanceTo(i);  // View function
                    break; // Escape the loop, we only needed one
                }
            }
        }
    }
